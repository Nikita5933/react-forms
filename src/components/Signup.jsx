...
function signupAction(prevFormState, formData) {
    ...
    if (!isEqualToOtherValue(password, passwordConfirm)) {
        errors.push('Password do not match');
    }
    if(!isNotEmpty(firstName) || !isNotEmpty(lastName)) {
        errors.push('First and last name are invalid');
    }
    if(!isNotEmpty(role)) {
        errors.push('Role is invalid');
    }
    if(!terms) {
        errors.push('Terms are invalid');
    }
    if(acquisitionChannel.length === 0) {
        errors.push('Acquisition Channel is invalid');
    }
